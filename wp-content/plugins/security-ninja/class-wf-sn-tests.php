...
class Wf_Sn_Tests extends WF_SN {
	...
	*/
	public static function admin_ssl() {
		$return = array();
	$testme = self::check_server_ssl();

	error_log('$testme ' . print_r($testme,true) );
		/*
		
		
		SSL = true + FORCE = true: 10 - great
		SSL = true + FORCE = false 10 - great (notice about FORCING)
		
		SSL = false + FORCE = true - 
		SSL = false + FORCE = false
		
		
		
		SSL not installed and FORCE = false = 0 fail
		SSL installed, and FORCE_ = false = 5
		
		
		SSL NOT INSTALLEd and FORCE_ = true =
		SLL INSTALLED and FORCE = false
		
		*/
		
		// checks if FORCE_ADMIN_SSL has been set.
		if ( ! defined( 'FORCE_SSL_ADMIN' ) ) {
			$return['status'] = 5;
			$return['details'] = 'FORCE_SSL_ADMIN not defined'; // @todo @i8n
		}
		
		if ( defined( 'FORCE_SSL_ADMIN' ) || ! FORCE_SSL_ADMIN ) {
			$return['status'] = 5;
			$return['details'] = 'FORCE_SSL_ADMIN defined, but not set to true'; // @todo @i8n
		}
		
		if ( false === stripos( get_home_url(), 'https' ) ) {
			$return['status'] = 5;
			$return['details'] = 'You should set your Settings -> General URLs to start with https://'; // @todo
		}
		
		$force_ssl_admin = force_ssl_admin();
		error_log( 'FORCE_SSL_ADMIN: '.print_r( $force_ssl_admin, true ) );
		
		if ( ! $force_ssl_admin ) {
			$return['status'] = 0;
			$return['details'] = 'Administration should be performed over SSL'; // @todo
		}
		
		if ( $force_ssl_admin ) {
			$return['status'] = 10;
			$return['details'] = 'Weeee!'; // @todo
		}
		/*
		
		if ( ( ! defined( 'FORCE_SSL_ADMIN' ) || ! FORCE_SSL_ADMIN ) && ( false === stripos( get_home_url(), 'https' ) ) ) {
			$return['status'] = 10;
			$return['details'] = 'her med 10';
		} else {
			$return['status'] = 5;
			$return['details'] = '5 her';
		}
